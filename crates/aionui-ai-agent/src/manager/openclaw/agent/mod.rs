...
impl OpenClawAgentManager {
    pub async fn new(
        ...
        let gateway_process = if !config.gateway.use_external_gateway {
            let cli_path = config
                ...
                .cli_path
                .as_deref()
                .ok_or_else(|| AgentError::BadRequest("OpenClaw CLI path is required".into()))?;
            ...
            if !is_port_listening(host, port).await {
                ...
                let process = Arc::new(CliAgentProcess::spawn_for_sdk(spawn_config, &data_dir).await?);
    ...
    async fn do_send_message(&self, is_first: bool, data: SendMessageData) -> Result<(), AgentError> {
        ...
        let session_key = self
            ...
            .session_key
            .clone()
            .ok_or_else(|| AgentError::Internal("No active session key".into()))?;
