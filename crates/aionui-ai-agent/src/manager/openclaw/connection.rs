...
impl OpenClawConnection {
    ...
    pub async fn close(&self) {
        ...
        for (_, tx) in pending.drain() {
            let _ = tx.send(Err(AgentError::Internal("Connection closed".into())));
        }
    }
    ...
    async fn run_reader(self: Arc<Self>, mut stream: WsStream) {
        ...
        let mut pending = self.pending.lock().await;
        for (_, tx) in pending.drain() {
            let _ = tx.send(Err(AgentError::Internal("OpenClaw connection closed".into())));
        }
    }
    ...
    async fn ws_send_frame(&self, frame: &RequestFrame) -> Result<(), AgentError> {
        ...
        let sink = guard
            .as_mut()
            .ok_or_else(|| AgentError::Internal("OpenClaw WebSocket not connected".into()))?;
